...
.param D = 1-((Vo+0.06)/(Vin-0.03))
*.param D = 1-((Vo)/(Vin-0.1))
.param TR = 0.01*T
.param TF = 0.01*T
.param TdR = 1n
.param TdF = 1n
...
compose fsw_sweep start = 500k stop = 15Meg step = 100k
...
foreach T_val $&T_sweep
	...
	*plot Ron_M1 Ron_M2
	...
	let loss_M1_sweep[index] = loss_M1
	let loss_M2_sweep[index] = loss_M2
	let sumaPot_sweep[index] = sumaPot
...
plot sumaPot_sweep vs fsw_sweep
.endc
...
"}
C {../DCDCBuck/DCDC_Buck.sym} -630 -220 0 0 {name=X1}
C {code.sym} -940 -320 0 0 {name=POWER_MOS_Param only_toplevel=false spice_ignore=0
