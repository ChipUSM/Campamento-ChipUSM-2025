...
N -490 -260 -490 -240 {
lab=#net1}
C {lab_pin.sym} -490 -318 0 0 {name=p1 sig_type=std_logic lab=Vdd}
C {lab_pin.sym} -594 -220 0 0 {name=p2 sig_type=std_logic lab=Vg_M1}
C {lab_pin.sym} -594 -180 0 0 {name=p3 sig_type=std_logic lab=Vg_M2}
C {lab_pin.sym} -346 -200 2 0 {name=p4 sig_type=std_logic lab=Vo}
C {vsource.sym} -270 -80 0 0 {name=Vin value=\{Vin\} savecurrent=false}
C {lab_pin.sym} -270 -140 0 0 {name=p5 sig_type=std_logic lab=Vdd}
C {gnd.sym} -270 -30 0 0 {name=l2 lab=GND}
C {vsource.sym} -550 70 0 0 {name=Vg2 value="PULSE(0 \{VH\} \{TdR\} \{TR\} \{TF\} \{T*D-TdR-TdF\} \{T\} 0)" savecurrent=false}
C {lab_pin.sym} -550 -140 0 0 {name=p6 sig_type=std_logic lab=Vg_M1}
C {gnd.sym} -550 -30 0 0 {name=l4 lab=GND}
C {vsource.sym} -550 -80 0 0 {name=Vg1 value="PULSE(0 \{VH\} 0 \{TR\} \{TF\} \{T*D\} \{T\} 0)" savecurrent=false}
C {lab_pin.sym} -550 10 0 0 {name=p7 sig_type=std_logic lab=Vg_M2}
C {gnd.sym} -550 120 0 0 {name=l5 lab=GND}
C {code.sym} -1010 -310 0 0 {name=Simulation_Parameters only_toplevel=false 
...
C {gnd.sym} -490 -150 0 0 {name=l3 lab=GND}
C {ammeter.sym} -490 -280 0 0 {name=V_Iin savecurrent=true spice_ignore=0}
C {launcher.sym} -510 -460 0 0 {name=h5
descr="load waves" 
tclcommand="xschem raw_read $netlist_dir/TB_DCDCBuck.raw tran"
}
C {../DCDCBuck/DCDC_Buck.sym} -480 -180 0 0 {name=X1}
C {code.sym} -810 -310 0 0 {name=MODEL only_toplevel=true spice_ignore=false
...
"}
C {devices/simulator_commands_shown.sym} -1680 -420 0 0 {name=COMMANDS spice_ignore =true
simulator=ngspice
only_toplevel=false 
value="
* ngspice commands
...
.endc
"}
C {devices/code.sym} -1230 -150 0 0 {name=Sweep_Io only_toplevel=false spice_ignore=true
...
compose fsw_sweep start = 1Meg stop = 10Meg step = 3Meg
...
.end
"}
C {devices/code.sym} -1580 30 0 0 {name=SweepTest only_toplevel=false spice_ignore=true
...
compose Io_sweep2 start = 0.5 stop = 2 step=0.1 
*compose R_sweep start=0.6 stop=2.4 step=0.2
compose i start=0 stop=14 step=1
let len = 1+(stp-strt)/step


let eff_sweep = vector(len)
let Io_sweep = vector(len)
let R_sweep = vector(len)
let Vo_sweep = vector(len)
let Po_sweep = vector(len)
let Pin_sweep = vector(len)
let loss_M1_sweep = vector(len)
...
let Io_inv = vector(len)

let index = 0

foreach Io_aux $&Io_sweep2
	let aux = \{Vo\}/$Io_aux
	print aux
	let R_sweep[index] = aux
	let index = index +1
end
print R_sweep

let R_2 = \{Vo\}/Io_sweep2
print R_2
...
.end
"}
C {devices/code.sym} -1100 -150 0 0 {name=Sweep_fsw only_toplevel=false spice_ignore=false
